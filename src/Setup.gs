...
function getSheetDefinitions_() {
  return [
    ...
    { name: SHEET.BLACKLIST,         headers: ['type', 'identifier', 'reason', 'added_at'] },
    { name: SHEET.NG_WORDS,          headers: ['word'] },
    { name: SHEET.EMOJIS,            headers: ['name', 'category', 'aliases', 'url', 'last_updated'] },
    { name: SHEET.NGRAM,             headers: ['prev_token', 'next_token', 'count', 'last_used_at'] },
    { name: SHEET.RAW_LEARN,         headers: ['note_id', 'platform', 'author_acct', 'content', 'learned_at', 'tokens_extracted'] },
    { name: SHEET.PROCESSED_MISSKEY, headers: ['note_id', 'processed_at', 'source'] },
    { name: SHEET.PROCESSED_MASTODON,headers: ['note_id', 'processed_at', 'source'] },
    { name: SHEET.POST_HISTORY,      headers: ['hash', 'posted_at', 'platform', 'original_text'] },
    { name: SHEET.FOLLOWS,           headers: ['platform', 'user_id', 'acct', 'is_follower', 'i_am_following', 'missing_count', 'followed_back_at', 'updated_at'] },
    { name: SHEET.DASHBOARD,         headers: ['date', 'platform', 'post', 'reply', 'follow_back', 'unfollow', 'learn', 'error', 'url_fetch', 'ngram_rows', 'followers', 'following'] },
    { name: SHEET.ERROR_LOG,         headers: ['timestamp', 'platform', 'function_name', 'error_message'] },
  ];
}
...
var CONFIG_DEFAULTS_ = [
  ...
  ['BOT_ACTIVE',                      'FALSE',     '全体有効化'],
  ['BOT_PLATFORM',                    'misskey',   'misskey / mastodon'],
  ['MISSKEY_INSTANCE',                '',          'URL(末尾スラなし)'],
  ['MISSKEY_WEBHOOK_ENABLED',         'TRUE',      'Webhook経由のメンション返信'],
  ['MASTODON_INSTANCE',               '',          'URL'],
  ...
  ['MORPH_URLFETCH_FALLBACK_THRESHOLD','15000',    'UrlFetch総数がこの値を超えたら簡易解析へ'],
  ...
  ['FOLLOW_AUTO_UNFOLLOW_BACK',       'FALSE',     'フォロー解除されたら自動アンフォロー'],
  ...
  ['NG_WORDS_EXTERNAL_URL',           'https://raw.githubusercontent.com/sayonari/goodBadWordlist/main/ja/BadList.txt', 'NGワード外部リスト URL'],
...
function validateConfig() {
  ...
  try {
    var sheet = getSheet_(SHEET.CONFIG);
    if (!sheet) {
      ui.alert('設定シートが見つかりません。先に「初期設定」を実行してください。');
      return;
    }
...
function manageApiTokens() {
  ...
  // Misskey トークン
  if (platform === 'misskey') {
    var mkRes = ui.prompt('Misskey API トークン', 'MISSKEY_TOKEN を入力してください:', ui.ButtonSet.OK_CANCEL);
    if (mkRes.getSelectedButton() === ui.Button.OK) {
      var mkToken = mkRes.getResponseText().trim();
      if (mkToken) {
        PropertiesService.getScriptProperties().setProperty('MISSKEY_TOKEN', mkToken);
  ...
  if (platform === 'mastodon') {
    var mdRes = ui.prompt('Mastodon API トークン', 'MASTODON_TOKEN を入力してください:', ui.ButtonSet.OK_CANCEL);
    if (mdRes.getSelectedButton() === ui.Button.OK) {
      var mdToken = mdRes.getResponseText().trim();
      if (mdToken) {
        PropertiesService.getScriptProperties().setProperty('MASTODON_TOKEN', mdToken);
  ...
  // Yahoo Client ID
  var yhRes = ui.prompt('Yahoo! Client ID', 'YAHOO_CLIENT_ID を入力してください:', ui.ButtonSet.OK_CANCEL);
  if (yhRes.getSelectedButton() === ui.Button.OK) {
    var yhId = yhRes.getResponseText().trim();
    if (yhId) {
      PropertiesService.getScriptProperties().setProperty('YAHOO_CLIENT_ID', yhId);
...
function testShowConfig() {
  ...
  var instance = getConfig(
    platform === 'mastodon' ? 'MASTODON_INSTANCE' : 'MISSKEY_INSTANCE', '(未設定)'
  );
  ui.alert(
    '現在の設定確認',
