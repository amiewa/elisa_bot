...
1. 作成したスプレッドシートを開く
2. メニュー **elisa_bot > 初期設定(シート作成)** を実行 — 13 シートが作成され、設定シートにデフォルト値が入力される
...
| NGカスタム絵文字 | 生成・登録から除外するカスタム絵文字（1行1絵文字名） |
| カスタム絵文字 | 絵文字名・URL・最終更新日時 |
...
**学習除外ブラックリスト** シートに登録したユーザーまたはインスタンスは、TL 学習の対象から除外されます。

### 列の説明

| 列 | 型 | 説明 |
|---|---|---|
| `type` | `user` または `instance` | 除外の種別。それ以外の値は無視される |
| `identifier` | 文字列 | 除外対象の識別子（書式は下記参照） |
| `reason` | 文字列 | 追加理由（自由記述・任意） |
| `added_at` | 日時 | 追加日時（任意） |

`reason` / `added_at` 列は処理に使われないメモ欄です。記入は任意です。

### identifier の書式

| type | 書式 | 例 |
|---|---|---|
| `user` | `@ユーザー名@ホスト` | `@spammer@example.tld` |
| `instance` | ドメイン名のみ（`https://` やパス無し） | `bad.example` |

- 大文字小文字は区別されません（`@Spam@Example.TLD` と `@spam@example.tld` は同一）。
- `instance` を指定すると、そのインスタンスの**全ユーザー**が除外されます。

### 記入例

| type | identifier | reason | added_at |
|---|---|---|---|
| `user` | `@spammer@example.tld` | スパム投稿が多い | 2026-06-07 |
| `instance` | `bad.example` | 低品質インスタンス | 2026-06-07 |

---

## NGカスタム絵文字

**NGカスタム絵文字** シートに登録した絵文字名は、文章生成での使用と絵文字一覧シートへの登録の両方から除外されます。

- 1行1絵文字名で記入します（コロン `:` は不要）。
- 大文字小文字は区別されません。
- 完全一致のみ除外します（`cat` を登録しても `cat_run` は除外されません）。
